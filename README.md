...
<img alt="my skills" src="https://skillicons.dev/icons?theme=dark&perline=7&i=php,html,css,python" />
...
## 結合テストにおいて苦労した点
結合テストの災対切り替え/切り戻しについて、AWS環境はCloudFormationテンプレートを用いて大半のリソースを作成することからCloudFormationを主で構築担当していた関係から、災対切り替え/切り戻しのメインの担当として対応しました。

私自身もAWS環境の災対切り替え/切り戻し対応は初めて対応するということもあり、事前にシナリオおよび手順の作成をしてはいたが、トライ&エラーで対応する箇所も多々ありました。

具体的には
- RDS (MySQL)のリストアが自動バックアップから災対リージョンでGUIで実行がうまくいかない -> 代替でCLI対応を実施
- AWS Backupから各EC2インスタンスをリストアする際に設計しているIPアドレスでリストアがうまくいかない -> メタデータのjsonファイルを編集しCLIで想定しているIPアドレス、その他ネットワーク環境にリストアされるように対応を実施

など、現在の事象がどうか、仮説としてどう対処していくべきか、などをPM/PLと相談の上対応を進めて結合テストを無事に完遂することができました。
...
- WAF Classicのマイグレーション対応
    - ｗAF Classicのサポート終了に伴いWAF v2へのマイグレーション対応を実施。
...
### AWS 問題集の解説図解経験
- [CloudTech](https://kws-cloud-tech.com/)が提供するAWS認定資格問題集の解説図解を実施。問題文および回答の解説文から要件に沿うようにGoogleスライドで1枚のスライドで図解を作成。ひと月おおよそ10枚程度の進捗で現在も対応中。
図解作成を継続的に実施することで、実務の設計や構成図作成業務を円滑に進めることができます。
